...
class Proxy
  ...
  def called?(some_method)
    @messages.include?(some_method)
  end

  def number_of_times_called(some_method)
    @messages.select { |x| x == some_method}.size
    # @messages.count(some_method) easier way ;)
  end
